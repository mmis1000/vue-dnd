...
export const DragLayer = defineComponent({
  setup (props, { slots }) {
    ...
    return () => {
      const items = provider.readonlyExecutions.filter(i => i.preview !== undefined && i.size !== undefined).map(i => {
        ...
        return <div style={wrapStyle} key={i.id}> {cloneVNode(i.preview!(), { style }) } </div>
      })
